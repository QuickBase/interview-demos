...
namespace QuickBase.Demo.Domain.Services
{
	...
	{
        private List<Tuple<string, int>> GetMockData()
        {
        ...
        public IEnumerable<CountryPopulation> GetCountryPopulations()
        {
			return GetMockData().Select(t => new CountryPopulation {
				Name = t.Item1,
				Population = t.Item2
			}).ToList();
        }

        public Task<IEnumerable<CountryPopulation>> GetCountryPopulationsAsync()
        {
